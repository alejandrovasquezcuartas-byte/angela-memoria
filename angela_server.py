...
def _send_whatsapp_message(text: str, to: str) -> Optional[dict]:
    ...
    try:
        ...
        if r.status_code >= 300:
            logger.warning(f"WA -> {to} {r.status_code} {r.text[:500]}")
        else:
            logger.info(f"WA enviado a {to} ({len(text)} chars)")
        return r.json()
    except Exception as e:
        logger.warning(f"WA error ({to}): {e}")
        return None

def _send_whatsapp_to_all(text: str):
    results = []
    if not (WHATSAPP_NOTIFY_TO and WHATSAPP_PHONE_ID and WHATSAPP_TOKEN):
        logger.info("WA no configurado; no se enviará mensaje.")
        return results
...
# WooCommerce config
# -----------------------------
WOO_BASE_URL = os.getenv("WOO_BASE_URL")  # p.ej. https://yavalva.com.co/wp-json/wc/v3
...
def _update_woocommerce_status(order_id: int, status: str = "on-hold") -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code >= 400:
            ...
            logger.info(f"Woo: orden {order_id} -> {status}")
        return resp.json()
    except Exception as e:
...
_CI_KEYS = [
    ...
    "cedula", "cédula", "dni", "document", "documento", "cc", "numero_documento", "nit",
    # prefijos de billing
    "billing_cedula", "billing_dni", "billing_document", "billing_documento", "billing_cc",
]

def _extraer_cedula(payload: Dict[str, Any]) -> str:
    billing = payload.get("billing") or {}
    # intentos directos en billing
    for k in _CI_KEYS:
        raw = billing.get(k)
        if raw:
            return str(raw)
    ...
    val = _get_meta_value(meta, [k.lower() for k in _CI_KEYS])
...
def _fmt_yavalva_whatsapp(payload: Dict[str, Any]) -> str:
    ...
    # Nota del cliente (Woo la expone como customer_note en REST)
    customer_note = (payload.get("customer_note") or "").strip()

    # Items (SKU si existe; si no, nombre)
    lines = []
...
async def webhook_woocommerce(
    ...
    logger.info(f"Webhook recibido (Woo) keys={list(payload.keys())[:10]}")
    db, _ = _db_bucket()
    ...
    logger.info(f"Pedido guardado: #{number} (doc={doc_id})")
